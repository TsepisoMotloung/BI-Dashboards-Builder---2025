...
class UploadService:
    ...
    def clean_dataframe_for_mysql(df: pd.DataFrame) -> pd.DataFrame:
        """Clean DataFrame to ensure all values are MySQL-compatible"""
        ...
        for col in df.columns:
            # Check if column has any NaN-like values that slipped through
            if df[col].dtype == 'object':
                # For object columns, ensure no nan strings
                df[col] = df[col].apply(lambda x: None if pd.isna(x) or (isinstance(x, float) and np.isnan(x)) else x)
            elif df[col].dtype in ['float64', 'float32']:
                # For float columns, check for NaN
                df[col] = df[col].apply(lambda x: None if pd.isna(x) or np.isnan(x) else x)
    ...
    async def upload_data(
        ...
        try:
            ...
            # CRITICAL FIX: Clean DataFrame for MySQL compatibility
            df = UploadService.clean_dataframe_for_mysql(df)
            print(f"DEBUG: DataFrame cleaned for MySQL")
            ...
            # Convert to records - with additional cleaning
            records = df.to_dict('records')
            
            # Final safety check: ensure no nan values in records
            cleaned_records = []
            for record in records:
                cleaned_record = {}
                for key, value in record.items():
                    # Triple-check for any nan values
                    if value is None:
